...
% arXiv version: 
\Figure{figures/W49_RGB_40kms_aplpy_small.png}
...
\FigureTwo{figures/G43.17+0.01_40kmscloud_MCMCfit_nolegend_MCMCoverplot.pdf}
...
The observed \formaldehyde line ratio roughly depends on the
...
%Since the observed optical depth is
%dependent on the number of particles at a given density, rather than the volume
%at that density, the observable is dependent on the mass-weighted density.
...
a logarithmic differential. %, $P_M\equiv(\rho/\rho_0) P_V$.
\begin{eqnarray}
...
% c_s: (((c.k_B*(10*u.K))/(c.m_p*2.37))**0.5).to('km/s')
% >>> (((c.k_B*(10*u.K))/(c.m_p*2.37))**0.5).to('km/s')
% <Quantity 0.18662445972 km / s>
% >>> (((c.k_B*(20*u.K))/(c.m_p*2.37))**0.5).to('km/s')
% <Quantity 0.263926842007 km / s>
% >>> (((c.k_B*(30*u.K))/(c.m_p*2.37))**0.5).to('km/s')
% <Quantity 0.323243046171 km / s>
% >>> (((c.k_B*(40*u.K))/(c.m_p*2.37))**0.5).to('km/s')
% <Quantity 0.373248919441 km / s>
% 
...
We use $T$ values given the $T-\sigma_s$ and $T-\mathcal{M}_C$
...
{\it Facilities:} \facility{GBT}, \facility{Arecibo},
