...
of the Initial Mass Function \citep[IMF;][]{Padoan2002a, Padoan2007a, Hennebelle2008a, Henebelle2009a, Chabrier2010a, 
...
\citep{Padoan2011b,Molina2012a}.\footnote{The details of this equation will be discussed in Section \ref{sec:turbulenceh2co},
but the relation is introduced here to facilitate further introduction.}
They characterize the turbulent driver in terms of its solenoidal (curly) versus compressive (convergent or divergent)
...
gravitational collapse in order to study the initial conditions of star formation.
...
``cloud-average'' versus \formaldehyde-based density measurements and left open
...
densities must be caused by the shape of the density distribution and infer the
...
\todo{Outline: Description of sections}
...
the GRS \thirteenco integrated image from $v_{LSR}=36$ \kms to $v_{LSR}=43$ \kms at contour levels of
...
\ref{fig:h2codensg43} and \ref{fig:w49fullspec} are labeled by their LSR
velocities, 7.84 and 16.30, for \north and \south respectively.}
...
$s\equiv\ln(\rho/\rho_0)$ for mean density $\rho_0 \equiv \meanrho_V$.
...
curl-free) forcing \citep{Federrath2008a,Federrath2010a}.  $c_s$ is the isothermal sound
speed, $v_A$ is the Alfvén speed, and $\mathcal{M}_A$ is the Alfvénic Mach
number.
...
$X(\ortho)=10^{-8.5}$, where the X-axis shows $\rho_0 \equiv \rho(\hh)$ and the Y-axis
...
ortho/para ratio 1.0.  The different lines show the effect of smoothing with different
...
Mach number based on the \north line of sight, $\mathcal{M}\approx5.1\pm1.5$.  
...
constrain $b$.  Figure \ref{fig:lognormalmcmc} shows the Mach number-$b$
...
distribution is incompatible with our observations for all $T$ values considered
in \citet{Hopkins2013a}.  Figure \ref{fig:distributions} shows how the Hopkins
and lognormal distributions differ, showing that the Hopkins distribution tends
to generate lower mass-weighted densities than the lognormal.  However, the
Hopkins distribution is compatible with our observations if a lower abundance,
$X(\formaldehyde)=10^{-9}$, is assumed.
...
The restrictions on $\sigma_s$ and $b$ using either assumed density
...
for fixed Mach number.  However, lower abundances than $X=10^{-9}$ are rarely
...
approximately corresponding to the volume-weighted mean density of \GRSMC from \thirteenco
...
$<\rho>_M$ and the width of the volume-weighted density PDF $P(\rho_V)$ for the
...
measured the width of the lognormal turbulence density distribution $\sigma_s
...
for Astronomy \citep{Astropy2013a}.  It also made use of pyspeckit, an open source
python-based package for spectroscopic data analysis \citep{Ginsburg2011c}.
