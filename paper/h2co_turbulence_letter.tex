...
This is an indication that any star formation, if present, is weak -- no
...
% 
% 
The cloud has mass $M_{CO} = 1.5\ee{4}$ \msun in a radius $r=15$ pc as measured
from the integrated map, so its mean density is $n(\hh) \approx 15$ \percc
assuming spherical symmetry.  If we
...
northmost cloud GRSMC 43.04-0.11 as an upper limit.  \todo{Using an ellipse that encompasses
...
$X(\ortho/\thirteenco) = 3.2\ee{-4}$ and 3.5\ee{-4} for G43.16 and G43.17
respectively, or relative to \hh, 5.8\ee{-10} and 6.2\ee{-10} \todo{TODO: Correct G43.16 numbers; G43.17 is accurate}, which are
...
ratio $\tau_{\oneone}/\tau_{\twotwo} = 6.99^{+0.72}_{-0.56} $.  Assuming a lognormal
distribution, and using only the $\tau$ measurements as a constraint, we derive the
