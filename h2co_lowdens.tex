...
% \kms;][]{Plume2004a},
so the 1-D Mach number of the turbulence is
$\mathcal{M}_{1D} \approx 5.1-9.1$ and $\mathcal{M}_{3D} \approx 8.7 - 15.8$.  
...
We infer a $T$ value from $T = 0.25 \ln (1+0.25 \sigma_s^4 (1+T)^{-6}$, where
$T$ is an ``intermittency'' parameter that indicates the deviation of the
distribution from lognormal.  Using the $\sigma_s=2.5$ distribution, which is
